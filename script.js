...
(() => {
   const inputHandler = () => {
     // determine maximum of characters
     const maxNrChars = MAX_CHARS;

     // dermine number of characters currently typed
...
(() => {
   ...
   const submitHandler = (event) => {
     ...
     // validate text (ex check if #hashtag is present and text is long enough)
     ...
     // we have text, new extract other info from text.
     ...
     fetch(`${BASE_API_URL}/feedbacks`, {
       ...
       .then((response) => {
         ...
         console.log('Successfylly submitted.');
       })
...
(() => {
   const clickHandler = (event) => {
     ...
     if (upvoteIntention) {
       ...
       upvoteCountEl.textContent = ++upvoteCount; // Shorthand very common... Increment
     } else {
...
(() => {
   const clickHandler = (event) => {
     ...
     feedbackListEl.childNodes.forEach((childNode) => {
       ...
       // extract compnay name
       const companyNameFromFeedbackItem = childNode
         .querySelector('.feedback__company')
         .textContent.toLowerCase()
         .trim();

       // remove feedback item from List if company names are not qqual.
